...
namespace Krono.IntegrationServices
{
    public class NettoImporter
    {
        private readonly HttpClient _httpClient;

        public NettoImporter(HttpClient httpClient)
        {
            _httpClient = httpClient;
        }


        public NettoResponse FromJson(string json)
        {
            return JsonConvert.DeserializeObject<NettoResponse>(json, Converter.Settings);
        }


        public async Task<List<Hit>> CallNetto(string query = "")
        {
            try
            {
                ...
                while (pagesToLoad > page)
                {
                    ...
                    var response = await _httpClient.PostAsync("/1/indexes/prod_NETTO_PRODUCTS/query", content);

                    if (response.IsSuccessStatusCode)
                    {
                        var responseStream = await response.Content.ReadAsStringAsync();
                        Console.WriteLine(responseStream);
                        NettoResponse r = FromJson(responseStream);
        ...
        internal class MatchLevelConverter : Newtonsoft.Json.JsonConverter
        {
